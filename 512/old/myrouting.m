function [child,judge,sum_parent]=myrouting(Solution,coor_pro,E_pro_node,xc,Y,D)
global rest_pro;
rest_pro=setdiff(Y,Solution);
global CN;
d0=20;           % single hop dis threshold
single_hop_set=[];
multi_hop_set=[];
xc=[-5,-5,5,5;-5,5,5,-5];
...
%     sum_parent(index_start(1))=sum_parent(index_start(1))+CN(index_start(1));    %start要向dest节点发送的包数量
    sum_parent(index_start(1))=sum_parent(index_start(1))+1;    %start要向dest节点发送的包数量
