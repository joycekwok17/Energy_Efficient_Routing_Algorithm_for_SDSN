...
% count death head nodes
a=1;
for i=1:S1
    if E_pro_node(i)>0
        Y(a)=i;         % Y数组存储alive pro nodes 
        a=a+1  ;        % 存活pro nodes数量为a-1
    end
end
...
for i=1:S2
    if E_com_node(i)>0
        W(b)=i;         % W数组存储alive common nodes序号
        b=b+1;          % 存活com nodes数量为b-1
    end
end
...
while (a>=S1+1)&&(b>=S2+1)     % a-1>0或b-1>0,网络中还有节点
     if (mod(k,300)==1) % 每x轮更新一次拓扑
         [Solution,E_pro_node,E_com_node,CN,Cluster,dis]=new_pso(S1,D,M,coor_com,coor_pro,E_pro_node,E_com_node,a,Y,W,xs,ys,xc,yc);
         [child,judge,parent]=new_routing(Solution,coor_pro,E_pro_node,Y,D,xc,yc,d0);
         [E_pro_node,E_com_node,sum_packet]=new_update(Solution,Y,W,D,coor_pro,E_pro_node,E_com_node,CN,Cluster,dis,Eda,sum_packet);
     else
         [E_pro_node,E_com_node,sum_packet]=new_update(Solution,Y,W,D,coor_pro,E_pro_node,E_com_node,CN,Cluster,dis,Eda,sum_packet);% sum_packet是从开始到第k轮为止的sum_packet
     end
   
     current_round_packet(k)=sum_packet*f; %总吞吐量
       
     a=1;
     for i=1:S1
         if E_pro_node(i)>0
                Y(a)=i;         % Y数组存储alive pro nodes 序号
                a=a+1;          % 存活pro node数为a-1
         end
     end
     for i=1:(a-1)
         E_alive_pro(i)=E_pro_node(Y(i));
     end
     ...
     for i=1:(b-1)
         E_alive_com(i)=E_com_node(W(i));
     end
     
     e1(k)=2*S1+0.5*S2-sum(E_alive_pro)-sum(E_alive_com);%消耗能量
     r1(k)=current_round_packet(k)/e1(k);%效率
     k=k+1                      % 此时生命长度为k-1轮
end
for i=1:1801-k
    add1(i)=r1(k-1);
...
%     
    ...
    if (dead==1)%第一个节点死亡
        if(flag_first_dead==0)  %第一个节点死亡周期
            ...
            xlabel('lifetime');ylabel('效率：吞吐量/能量消耗');
            ...
            plot(1:rmax,sum_packet,'g-o','linewidth',1.5,'markersize',2.5);   hold on;    %吞吐量
            xlabel('lifetime');ylabel('吞吐量');
            legend('NWPSO-based','Leach')
            hold on;
        end
    end
