...
for i=1:D
    E_pro_node(Solution(i))= E_pro_node(Solution(i))- CN(i)*f*(Eelec+Eda);
    % 头节点接收成员节点发来的数据包，能耗
...
for i=1:(length(W))             % 普通节点向CH发送数据包的能耗
...
[child,judge,parent]=myrouting(Solution,coor_pro,E_pro_node,xc,Y,D);
